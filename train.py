...
def evaluate(model, data_loader, scaler, args, device, epoch=0, total_sample=1):
    ...
    plot_results(meta, output, device, args, epoch, "test_")
...
def train_one_epoch(
    ...
    model.eval()
    plot_results(meta, output, device, args, epoch, "train_")
    del meta, output, loss


def plot_results(meta, output, device, args, epoch, prefix=""):
    with torch.inference_mode():
        if args.model == "phasenet":
