...
picks_path = Path(f"../../{dataset_name}_16/")
output_path = Path(f"{dataset_name}/picks_phasenet_filtered/")
figures_path = Path(f"{dataset_name}/figures_phasenet_filtered/")
...
# %%
stations_north = pd.read_csv("/net/kuafu/mnt/tank/data/EventData/Mammoth_north/das_info.csv", index_col="index")
stations_south = pd.read_csv("/net/kuafu/mnt/tank/data/EventData/Mammoth_south/das_info.csv", index_col="index")
stations
...
if __name__ == "__main__":
    ...
    files = list(picks_path.rglob('picks_phasenet_das_merged/*.csv'))
    ...
    events.to_csv(picks_path.joinpath("catalog_gamma.csv"), index=False, columns=["event_id", "time", "longitude", "latitude","depth_km","event_index"], float_format="%.6f")
